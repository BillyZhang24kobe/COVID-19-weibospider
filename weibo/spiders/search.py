    ...
    def parse_weibo(self, response):
        ...
        for sel in response.xpath("//div[@class='card-wrap']"):
            ...
            if info:
                ...
                weibo['retweet_text'] = ''
                weibo['retweet_location'] = ''
                weibo['retweet_topics'] = ''
                weibo['retweet_reposts_count'] = ''
                weibo['retweet_comments_count'] = ''
                weibo['retweet_attitudes_count'] = ''
                weibo['retweet_created_at'] = ''
                weibo['retweet_source'] = ''

                if retweet_sel and retweet_sel[0].xpath(
                        './/div[@node-type="feed_list_forwardContent"]/a[1]'):
                    # retweet = WeiboItem()
                    weibo['retweet_id'] = retweet_sel[0].xpath(
                        './/a[@action-type="feed_list_like"]/@action-data'
                    ).extract_first()[4:]

                    weibo['retweet_bid'] = retweet_sel[0].xpath(
                    ...
                    weibo['retweet_user_id'] = info.xpath(
                        '@href').extract_first().split('/')[-1]

                    weibo['retweet_screen_name'] = info.xpath(
                        '@nick-name').extract_first()

                    weibo['retweet_text'] = retweet_txt_sel.xpath(
                        'string(.)').extract_first().replace('\u200b',
                                                             '').replace(
                                                                 '\ue627', '')

                    location = self.get_location(retweet_txt_sel)
                    weibo['retweet_location'] = location
                    if location:
                        weibo['retweet_text'] = weibo['retweet_text'].replace(
                            '2' + location, '')
                    weibo['retweet_text'] = weibo['retweet_text'][2:].replace(' ', '')
                    if is_long_retweet:
                        weibo['retweet_text'] = weibo['retweet_text'][:-6]

                    weibo['retweet_topics'] = self.get_topics(retweet_txt_sel)
                    reposts_count = retweet_sel[0].xpath(
                        './/ul[@class="act s-fr"]/li/a[1]/text()'
                    ).extract_first()
                    reposts_count = re.findall(r'\d+.*', reposts_count)
                    weibo['retweet_reposts_count'] = reposts_count[
                        0] if reposts_count else '0'
                    comments_count = retweet_sel[0].xpath(
                        './/ul[@class="act s-fr"]/li[2]/a[1]/text()'
                    ).extract_first()
                    comments_count = re.findall(r'\d+.*', comments_count)
                    weibo['retweet_comments_count'] = comments_count[
                        0] if comments_count else '0'
                    attitudes_count = retweet_sel[0].xpath(
                        './/a[@action-type="feed_list_like"]/em/text()'
                    ).extract_first()
                    weibo['retweet_attitudes_count'] = (attitudes_count
                                                  if attitudes_count else '0')
                    created_at = retweet_sel[0].xpath(
                        './/p[@class="from"]/a[1]/text()').extract_first(
                        ).replace(' ', '').replace('\n', '').split('前')[0]
                    weibo['retweet_created_at'] = util.standardize_date(created_at)
                    source = retweet_sel[0].xpath(
                        './/p[@class="from"]/a[2]/text()').extract_first()
                    weibo['retweet_source'] = source if source else ''
